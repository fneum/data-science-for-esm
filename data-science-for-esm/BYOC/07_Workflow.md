...
=========================================================
In [Deployment Settings](./05_Deploy.md#deployment-settings) we defined the deployment branch in the `.github/workflows/deploy.yml` [file][deploy], which is used in the workflow, whenever performing `git push`. You are free to use any branch, other than the `gh-pages`, keeping in mind that only upon the `git push`, the workflow will be triggered.
